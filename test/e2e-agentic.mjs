...
const explicitInboundApiToken = process.env.CURSOR2API_INBOUND_API_TOKEN || process.env.API_AUTH_TOKEN || process.env.API_TOKEN;
const INBOUND_API_TOKEN = explicitInboundApiToken || 'dummy';
const AUTH_HEADERS = { 'Content-Type': 'application/json', 'x-api-key': INBOUND_API_TOKEN };
const AUTH_ONLY_HEADERS = { 'x-api-key': INBOUND_API_TOKEN };
...
console.log(info(`  inbound token=${explicitInboundApiToken ? 'configured' : 'using fallback dummy'}`));

try {
    const r = await fetch(`${BASE_URL}/v1/models`, { headers: AUTH_ONLY_HEADERS });
