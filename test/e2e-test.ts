...
const INBOUND_API_TOKEN = explicitInboundApiToken || 'test';
const AUTH_HEADERS = { 'Content-Type': 'application/json', 'x-api-key': INBOUND_API_TOKEN };

console.log(`[e2e-test] server: ${BASE_URL}`);
console.log(`[e2e-test] inbound token: ${explicitInboundApiToken ? 'configured' : 'using fallback test token'}`);
