...
编辑 `config.yaml` 时只放**非 secret 的基础配置**：
...
- `vision.mode` - 视觉模式。推荐 `ocr`；如需真视觉理解改为 `api`
- `vision.providers` - API 模式下按顺序尝试的外部视觉 provider 列表

运行时 secret 统一走环境变量，不要写进 `config.yaml`：
- 入站鉴权：`CURSOR2API_INBOUND_API_TOKEN`
- ModelScope：`MODELSCOPE_API_KEY` / `MODELSCOPE_API_TOKEN`
- 可选覆盖：`MODELSCOPE_BASE_URL`、`MODELSCOPE_MODEL`
- 通用视觉覆盖：`VISION_ENABLED`、`VISION_MODE`、`VISION_FALLBACK_TO_OCR`、`VISION_BASE_URL`、`VISION_API_KEY`、`VISION_MODEL`

推荐的 secret 放置方式：
- **本地 / Claude Code**：放进 `~/.env_secrets`
- **Hugging Face Spaces**：放进 **Space Secrets**
- **VPS / Docker / PM2 / systemd**：通过运行时环境变量注入

> `src/config.ts` 的代码默认值不等于当前部署默认值。如果要把运行时默认模型切到 `Qwen/Qwen3.5-27B`，直接设置 `MODELSCOPE_MODEL=Qwen/Qwen3.5-27B`，不需要改 `src/config.ts` / `src/vision.ts`。

### 3. 启动

```bash
source ~/.env_secrets
export CURSOR2API_INBOUND_API_TOKEN="<your-inbound-token>"
npm run dev
```

如需启用 ModelScope 视觉 API 模式，可再设置：

```bash
export VISION_MODE=api
export MODELSCOPE_API_KEY="<your-modelscope-token>"
export MODELSCOPE_BASE_URL="https://api-inference.modelscope.cn/v1/chat/completions"
export MODELSCOPE_MODEL="Qwen/Qwen3.5-27B"
```
...
export ANTHROPIC_AUTH_TOKEN="$CURSOR2API_INBOUND_API_TOKEN"
claude
```
...
OPENAI_API_KEY=<与 CURSOR2API_INBOUND_API_TOKEN 相同的入站 token>
```
模型选择 `claude-sonnet-4-20250514` 或其他列出的 Claude 模型名。

> ⚠️ **注意**：Cursor IDE 请优先选用 Claude 模型名（通过 `/v1/models` 查看），避免使用 GPT 模型名以获得最佳兼容。

## Hugging Face Spaces 部署与安全说明

### 必备 Secrets

Hugging Face Spaces 部署时，至少把以下内容放进 **Space Secrets**：
- `CURSOR2API_INBOUND_API_TOKEN`
- `MODELSCOPE_API_KEY` 或 `MODELSCOPE_API_TOKEN`
- 可选：`MODELSCOPE_BASE_URL`
- 可选：`MODELSCOPE_MODEL`

### 鉴权行为

- 受保护接口：`/v1/messages`、`/messages`、`/v1/chat/completions`、`/chat/completions`、`/v1/responses`、`/responses`、`/v1/messages/count_tokens`、`/messages/count_tokens`、`/v1/models`
- 支持请求头：`Authorization: Bearer <token>` 或 `x-api-key: <token>`
- `GET /` 与 `GET /health` 保持开放
- 未配置入站 token 时服务会直接拒绝启动，不再匿名暴露

### HF 向 code review 结论

- 现状最重要的安全缺口是此前无入站鉴权，公开 Space 会直接暴露代理接口
- `config.yaml` 会进入镜像，因此只能放非 secret 默认配置
- `.dockerignore` 已排除 `refs/`、`test/`、`debug_payload.js`、`last_payload.json`，减少构建上下文冗余
- HF Space 应只保留运行所需内容；测试、调试和参考资料不应进入 Space 仓库
- HF CPU 环境下，本地 OCR 更适合作为兜底；若主要依赖多模态理解，优先使用 `vision.mode=api` + Space Secrets

### HF 最小 smoke test

未带 token 时，受保护接口应返回 `401`：

```bash
curl -i https://<your-space>.hf.space/v1/models
```
...
```bash
curl -sS \
  -H "Authorization: Bearer $CURSOR2API_INBOUND_API_TOKEN" \
...
curl -sS \
  ...
  https://<your-space>.hf.space/v1/messages
```
